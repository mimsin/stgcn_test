...
from models.st_tag import ST_TAG
...
def model_train(train_dataloader, val_dataloader, config, device):
    ...
    elif(config['type_model'] == "TAGCN"):
        model = ST_TAG(in_channels=config['N_HIST'], out_channels=config['N_PRED'], n_nodes=config['N_NODE'], k=config['k'], bias=config['bias'], normalize=config['normalize'], dropout=config['DROPOUT'])
        optimizer = optim.Adam(model.parameters(), lr=config['INITIAL_LR'], weight_decay=config['WEIGHT_DECAY'])
        loss_fn = torch.nn.MSELoss
        model.to(device)

    elif(config['type_model'] == "GCN"):
        model = ST_GCN(in_channels=config['N_HIST'], out_channels=config['N_PRED'], n_nodes=config['N_NODE'], bias=config['bias'], normalize=config['normalize'],
                       dropout=config['DROPOUT'], add_self_loops=config['add_self_loops'], cached=config['cached'], improved=config['improved'])
        optimizer = optim.Adam(model.parameters(), lr=config['INITIAL_LR'], weight_decay=config['WEIGHT_DECAY'])
        loss_fn = torch.nn.MSELoss
        model.to(device)
    ...
    torch.save({
            ...
            "loss": loss,
            }, os.path.join(config["CHECKPOINT_DIR"], f"model_{timestr+'_'+config['type_model']+'_epoch_'+str(config['EPOCHS'])}.pt"))
...
def model_test(model, test_dataloader, device, config):
    ...
    :param device Device to evaluate on
    """
    _, _, _, y_pred, y_truth = eval(model, device, test_dataloader, config['type_model'])
    day0_truth, day0_pred, t = plot_prediction(test_dataloader, y_pred, y_truth, 0, config)
    return day0_truth, day0_pred, t
...
def plot_prediction(test_dataloader, y_pred, y_truth, node, config):
    ...
    # plt.plot(t, day0_pred, label=config['type_model'])
...
def load_from_checkpoint(checkpoint_path, config):
    ...
    if(config['type_model'] == "GAT"):
        ...
        model = ST_GCN(in_channels=config['N_HIST'], out_channels=config['N_PRED'], n_nodes=config['N_NODE'], bias=config['bias'], normalize=config['normalize'], dropout=config['DROPOUT'],
                       add_self_loops=config['add_self_loops'], cached=config['cached'], improved=config['improved'])
        checkpoint = torch.load(checkpoint_path, map_location=torch.device('cpu'))
        model.load_state_dict(checkpoint['model_state_dict'])
